...
export async function lastfmEventFullLineup(eventUrl, { timeoutMs = 5000, maxPages = 12 } = {}) {
  ...
  const baseAbs = eventUrl.startsWith('http')
    ? eventUrl
  ...
  //
  // CRITICAL: each page gets its OWN AbortSignal timeout. The caller's
  // timeoutMs is treated as PER-PAGE, not wall-clock for the whole
  // walk. Big festivals (Brutal Assault has 8 pages, 115+ acts) need
  // ~5-10s end-to-end and would otherwise abort on slow LFM responses,
  // leaving the user with only the first 1-2 pages = first 15-30 acts.
  //
  // Partial result discipline: a fetch error mid-walk DOES NOT discard
  // the names we've already collected. We return what we have and let
  // the caller decide whether to use it (importer falls back to the
  // shorter listing-lineup if our partial is shorter, but doesn't
  // re-import a worse version of an already-imported lineup).
  if (isFestival) {
    const out = [];
    const seen = new Set();
    let lastFound = -1;
    for (let p = 1; p <= maxPages; p++) {
      ...
      try {
        ...
        // Page timed out or network blip — keep what we already have
        // and stop walking. Better to return 60 known bands than throw
        // away the whole walk over one slow page.
        break;
      }
      if (!res.ok) {
        // 429 / 503 → bail with partial. Other 4xx → also stop.
        break;
      }
      const html = await res.text().catch(() => '');
      const pageNames = extractBandNames(html);
      const fresh = pageNames.filter(n => !seen.has(n.toLowerCase()));
      if (fresh.length === 0) break;       // exhausted
      for (const n of fresh) { seen.add(n.toLowerCase()); out.push(n); }
      // Last.fm shows ~15-17 acts per page; if a page returned fewer
      // it's likely the final page. Two consecutive low pages = stop.
      if (fresh.length < 10 && lastFound < 10) break;
