...
async function fetchLastfmUserEventsPage(username, page, { past, timeoutMs }) {
  ...
  } catch { return { events: [], end: true }; }
  if (!res.ok) return { events: [], end: true };
  const html = await res.text().catch(() => '');
  if (!html) return { events: [], end: true };

  const blocks = extractJsonLdBlocks(html);
  const raw    = collectMusicEvents(blocks);
  // No artist filter for user-events — the headline performer is
  // whoever Last.fm lists first. mapLastfmEvent will unshift a name
  // we pass in; for user-events we pass '' so the lineup stays as-is.
  const events = raw.map(e => mapLastfmEvent(e, '')).filter(Boolean);
  // Heuristic for "end of pagination": no events on a page AND the
  // HTML doesn't contain a "next page" link. Cheaper than parsing
  // pagination metadata. Build the next-page regex dynamically since
  // we don't know `page` at literal-write time.
  const nextPageRx = new RegExp('page=' + (page + 1) + '[^0-9]');
  const hasNext = /class="[^"]*pagination-next[^"]*"/i.test(html) || nextPageRx.test(html);
  return { events, end: events.length === 0 && !hasNext };
}

export async function lastfmUserEventsAll(username, { past = true, maxPages = 30, timeoutMs = 7000, pacingMs = 350 } = {}) {
  const name = String(username || '').trim();
  if (!name) return [];
  const out = [];
  for (let page = 1; page <= maxPages; page++) {
    const { events, end } = await fetchLastfmUserEventsPage(name, page, { past, timeoutMs });
    out.push(...events);
    if (end) break;
    // Pace politely so we don't get rate-limited mid-import.
    if (page < maxPages) await new Promise(r => setTimeout(r, pacingMs));
  }
  return out;
}
