...
// ── Event detail page → full lineup ───────────────────────────
//
// The events-listing page truncates lineups to 3-5 names for layout.
// Festivals + bigger gigs hide the rest behind a "+ N more" link
// on the actual event detail page (/event/{id} or /festival/{id}).
// This helper fetches that page and extracts every performer name
// via the schema.org `itemprop="performer"` markup Last.fm renders
// for SEO. Each performer block looks like:
//
//   <span itemprop="performer" itemscope itemtype=".../MusicGroup">
//     <a href="/music/Kataklysm">Kataklysm</a>
//   </span>
//
// We pull every <a itemprop="url">NAME</a> inside one of those blocks.
// Returns array of performer names (empty on failure / 404).
export async function lastfmEventFullLineup(eventUrl, { timeoutMs = 5000 } = {}) {
  if (!eventUrl) return [];
  // Accept both /event/123+Name and https://www.last.fm/event/... shapes.
  const url = eventUrl.startsWith('http')
    ? eventUrl
  ...
  const names = [];
  // Walk every performer block. The itemprop="performer" attribute can
  // appear on the wrapping <span> or <div>. We grab the next <a> with
  // itemprop="url" inside as the canonical name (more reliable than
  // <p itemprop="name"> which sometimes has trailing whitespace).
  const performerRx = /<(?:span|div|li)[^>]*itemprop="performer"[\s\S]*?<a[^>]*itemprop="url"[^>]*>([\s\S]*?)<\/a>/gi;
  let m;
  while ((m = performerRx.exec(html)) !== null) {
    const name = m[1].replace(/<[^>]+>/g, '').trim();
    if (name && !names.some(n => n.toLowerCase() === name.toLowerCase())) names.push(name);
  }
  // Fallback: <p itemprop="name"> blocks if the URL-anchor pattern
  // didn't match (older Last.fm pages renderowane differently).
  if (names.length === 0) {
    const nameRx = /<p[^>]*itemprop="name"[^>]*>([\s\S]*?)<\/p>/gi;
    let mn;
    while ((mn = nameRx.exec(html)) !== null) {
      const n = mn[1].replace(/<[^>]+>/g, '').trim();
