...
function parseLastfmHtmlEvents(html, fallbackArtist) {
  ...
  // Accept multiple wrapper elements — Last.fm shipped at least three
  // distinct DOM structures for events lists over the years (<tr>,
  // <div>, <li>) plus the modern artist-page <article>. We capture
  // the opening tag name with backref \1 so the matching closer is
  // type-correct. Skipping the legacy `<tr>`-only matcher meant
  // 2010-era markup variants returned 0 events on import.
  const rowRx = /<(tr|div|li|article)[^>]*class="[^"]*\bevents-list-item\b[^"]*"[^>]*>([\s\S]*?)<\/\1>/gi;
  const events = [];
  let m;
