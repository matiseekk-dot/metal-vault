...
// Retry-with-backoff fetch. Last.fm rate-limits aggressively under
// repeated scrapes: a busy importer hits 429 (rate-limit) or 503
// (overloaded) several times across a 15-year archive walk. Bailing
// on the first error means most importer runs return partial data
// and the user has to retry repeatedly (each run claws back a few
// more pages as rate-limit windows reset).
//
// Strategy: classify failures into RETRYABLE (timeout, 429, 503, 504,
// network errors) vs HARD (404 / 410 / 4xx other than 429). On
// retryable, sleep 800ms × attempt and retry up to `retries` times.
// On hard, return immediately with the failure response.
//
// Caller still sees a single Response (or rejection) — the retry is
// invisible. Returns the FINAL response, retryable-failed or not, so
// the caller's existing not-ok handling still fires when we exhaust
// retries.
async function fetchLfm(url, { timeoutMs = 8000, retries = 2 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // Retryable HTTP — sleep + try again.
  ...
  // Should be unreachable; we always return inside the loop or throw.
...
export async function lastfmEventFullLineup(eventUrl, { timeoutMs = 5000, maxPages = 12 } = {}) {
  ...
  if (isFestival) {
    ...
    for (let p = 1; p <= maxPages; p++) {
      ...
      try {
        ...
        res = await fetchLfm(u, { timeoutMs, retries: 2 });
      } catch (e) {
  ...
  let res;
  try {
    res = await fetchLfm(cleanedBase, { timeoutMs, retries: 2 });
  } catch { return []; }
  if (!res.ok) return [];
  const html = await res.text().catch(() => '');
...
async function fetchLastfmUserEventsPage(username, page, { yearOrPath, timeoutMs }) {
  ...
  try {
    ...
    res = await fetchLfm(url, { timeoutMs, retries: 2 });
