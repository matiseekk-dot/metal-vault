...
export function discogsApiHeaders(oauthTokenObj) {
  ...
  // Only use OAuth if access_token is a real token (not the 'manual_token' placeholder)
  const isRealToken = oauthTokenObj?.access_token &&
                      oauthTokenObj?.access_secret &&
                      oauthTokenObj.access_token !== 'manual_token';
  if (isRealToken && key && secret) {
