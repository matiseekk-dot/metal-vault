...
// Detects the Capacitor native wrapper. Capacitor and TWA are two
// different mobile shells — the existing isPlayStoreTWA() check uses
// document.referrer (set by Bubblewrap TWAs), which is empty in the
// Capacitor WebView. We need a separate probe so the payment flow
// can route correctly inside the Capacitor app.
...
async function startStripeCheckout(plan) {
  try {
    ...
    if (d.url) {
      // In the Capacitor app, open Stripe Checkout in the system
      // browser (Chrome / Edge). Two reasons:
      //  1. Stripe's 3-D Secure challenge doesn't reliably render
      //     inside the embedded WebView — issuer banks pop iframes
      //     with cross-origin storage requirements that the WebView
      //     blocks, leaving the user with a blank screen mid-payment.
      //  2. Google Play policy forbids non-Play in-app purchases for
      //     apps distributed through the Play Store. Routing Stripe
      //     externally keeps the app compliant. Native Google Play
      //     Billing (Capacitor + RevenueCat) is planned for v1.1.
      if (isCapacitorApp()) {
        window.open(d.url, '_system');
        return { success: true, opened: 'external' };
      }
