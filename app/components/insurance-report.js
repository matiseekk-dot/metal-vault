...
export async function generateInsuranceReport({ collection, profile, ownerInfo }) {
  ...
  const brandHost = (process.env.NEXT_PUBLIC_APP_URL || (typeof window !== 'undefined' ? window.location.origin : ''))
    .replace(/^https?:\/\//, '');
  doc.text('Generated by Metal Vault  •  ' + brandHost, pageW - 40, pageH - 40, { align: 'right' });
