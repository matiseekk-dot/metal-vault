...
export async function GET() {
  ...
  try {
    ...
    const fetches = styles.map(style =>
      fetch('https://api.discogs.com/database/search?type=release&format=Vinyl&style='
        + encodeURIComponent(style) + '&sort=year&sort_order=desc&per_page=20&page=1',
    ...
    for (let i = 0; i < responses.length; i++) {
      for (const item of (responses[i].results || [])) {
        ...
        results.push({
          ...
          genre: item.style?.[0] || styles[i],
          spotifyUrl: '', discogsUrl: 'https://www.discogs.com' + (item.uri || ''),
    ...
    for (let i = 0; i < responses.length; i++) {
      for (const item of (responses[i].results || [])) {
        ...
        results.push({
          ...
          genre: item.style?.[0]||styles[i],
          spotifyUrl:'', discogsUrl:'https://www.discogs.com'+(item.uri||''),
          format: item.format?.join(', ')||'Vinyl', country:item.country||'', label:item.label?.[0]||'',
        });
      }
    }
    if (!results.length) throw new Error('No results from Discogs');
