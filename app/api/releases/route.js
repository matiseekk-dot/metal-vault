...
// ── Fetch releases for one genre query ────────────────────────
async function fetchGenre(token, query, limit = 20) {
  const url = `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=album&market=US&limit=${limit}`;
  const r = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  const d = await r.json();
  return d.albums?.items || [];
}

// ── Mock fallback (when Spotify not configured) ───────────────
const MOCK = [
  ...
  { id:'m15',artist:'Yob',              album:'Our Raw Heart',               cover:null, releaseDate:'2018-06-08', genre:'Doom Metal', spotifyUrl:'' },
...
// ── Normalise Spotify album → our format ─────────────────────
