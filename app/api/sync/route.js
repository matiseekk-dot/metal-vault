...
import { createServerSupabaseClient, getAdminClient } from '@/lib/supabase-server';

// Build OAuth header using user's personal access token
...
function buildHeader(discogsToken) {
  // discogsToken = { access_token, access_secret } from discogs_tokens table
  const key    = process.env.DISCOGS_KEY;
  const secret = process.env.DISCOGS_SECRET;
  const token  = process.env.DISCOGS_TOKEN; // fallback personal token
...
export async function POST(req) {
  ...
  const { type = 'both' } = body; // 'collection' | 'wantlist' | 'both'
  ...
  if (!username) return NextResponse.json({ error: 'No Discogs username found — reconnect Discogs' }, { status: 400 });
  ...
  if (type === 'collection' || type === 'both') {
    try {
      ...
      for (const r of raw) {
        ...
        if (existing) {
          // Update cover/format but preserve user-set price and grade
          await admin.from('collection').update({
            ...
            genres:     item.genres,
            styles:     item.styles,
            // Only update purchase_price if not already set by user
